...
In this tutorial we're going to write some unit tests for a Celo smart contract.

Writing tests is an important part of smart contract development because smart contract vulnerabilities can have real world implications like losing user funds! The best way to try to mitigate vulnerabilities is to write unit tests to test every part of our contract's functionality. 
...
Next, we're going to setup our project. Let's create a directory called `celoTesting` and `cd` into the folder: 

`mkdir celoTesting && cd celoTesting`

Next, let's get NPM setup: 

`npm init`

The project defaults will work fine for this. Now that we have NPM, let's initialize Truffle: 

`truffle init`

There should now be a handful of files and folders in your `celoTesting` directory. Let's open the folder with your favorite text editor and continue. 
...
Create a new contract in the `contracts/` folder named `HelloWorld.sol` and write the following: 
...
Great! This is just a simple smart contract which will either get the name variable by calling `getName()` or set a new name by calling `setName()`. When the contract first runs, `name` will be Celo.
...
To spin up this local blockchain, in a new terminal window run: 

`ganache-cli --port 8545`

We use a new terminal window since we'll want to keep Ganache running while we develop. 

Next, update the ``truffle.config`` file to work on truffle. Replace the existing contents with: 
...
```
This will simply use the Ganache environment whenever we deploy or test our contracts. 
